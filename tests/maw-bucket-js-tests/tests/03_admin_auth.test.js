...
describe('Admin Autentikasi', function () {
  ...
  describe('Tampilan Halaman Login', () => {
    ...
    it('TC-AUTH-02: Field email dan password tersedia', async () => {
      ...
      assert.ok(await email.isDisplayed(), 'Field email tidak tampil');
      assert.ok(await pass.isDisplayed(),  'Field password tidak tampil');
      assert.ok(await btn.isDisplayed(),   'Tombol submit tidak tampil');
    });
