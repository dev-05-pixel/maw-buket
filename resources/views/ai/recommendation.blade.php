...
<style>
    ...
    .ai-hero {
        ...
        position: relative;
        overflow: hidden;
        padding: 0 clamp(24px, 6vw, 100px) clamp(48px, 7vw, 80px);
    ...
    .ai-hero-title-inner:nth-child(1) .ai-hero-title-line { animation-delay: 0.5s; }
    .ai-hero-title-inner:nth-child(2) .ai-hero-title-line { animation-delay: 0.65s; }
...
<header class="ai-hero" aria-label="AI Recommendation hero">
    ...
    <div class="ai-hero-content">
        <div class="ai-badge" aria-label="Powered by AI">
        ...
        <p class="ai-hero-sub">
            Sistem kecerdasan buatan kami menganalisis budget, acara, dan preferensi warnamu untuk menemukan buket yang paling cocok dari koleksi Maw Bouquet.
        </p>
    </div>
</header>
    ...
    <div class="ai-form-panel reveal">
        ...
        <form method="POST" action="{{ route('ai.process') }}" id="ai-form">
            ...
            <div class="form-panel-body">
                ...
                <div class="ai-field">
                    <div class="ai-field-header">
                        ...
                        <label class="skip-toggle" for="skip-budget" title="Abaikan kriteria budget">
                            <input type="checkbox" id="skip-budget" name="skip_budget" value="1" class="skip-checkbox"
                                   data-target="inp-budget" data-slider="budget-slider"
                    ...
                    <div class="budget-display">
                        <span class="budget-value" id="budget-display">
                            Rp <span id="budget-val">{{ old('budget', 150000) ? number_format(old('budget', 150000), 0, ',', '.') : '150.000' }}</span>
                        </span>
                    ...
                    <input type="range" class="budget-slider" id="budget-slider"
                           ...
                           {{ old('skip_budget') ? 'disabled' : '' }} />

                    <input type="hidden" name="budget" id="inp-budget" value="{{ old('budget', 150000) }}" />

                    <p class="ai-input-hint">Seret slider untuk menyesuaikan budget. Pilih "Skip" untuk mengabaikan filter ini.</p>
                </div>
                ...
                <div class="ai-field">
                    <div class="ai-field-header">
                        <label class="ai-field-label" for="inp-acara">Acara / Momen</label>
                        <label class="skip-toggle" for="skip-acara" title="Abaikan kriteria acara">
                            <input type="checkbox" id="skip-acara" name="skip_acara" value="1" class="skip-checkbox"
                                   data-target="inp-acara"
                                   {{ old('skip_acara') ? 'checked' : '' }} />
                            <div class="skip-toggle-track">
                                <div class="skip-toggle-knob"></div>
                            </div>
                            <span class="skip-label-text">Skip</span>
                    ...
                    <div class="ai-select-wrap">
                        <select name="acara" id="inp-acara" class="ai-select"
                                {{ old('skip_acara') ? 'disabled' : '' }}>
                            <option value="ulang_tahun"   {{ old('acara') == 'ulang_tahun'   ? 'selected' : '' }}>🎂 Ulang Tahun</option>
                            <option value="romantis"      {{ old('acara') == 'romantis'      ? 'selected' : '' }}>💑 Romantis / Anniversary</option>
                            <option value="pernikahan"    {{ old('acara') == 'pernikahan'    ? 'selected' : '' }}>💍 Pernikahan / Wedding</option>
                            <option value="simpati"       {{ old('acara') == 'simpati'       ? 'selected' : '' }}>🕊️ Simpati / Duka Cita</option>
                            <option value="wisuda"        {{ old('acara') == 'wisuda'        ? 'selected' : '' }}>🎓 Wisuda / Kelulusan</option>
                            <option value="sederhana"     {{ old('acara') == 'sederhana'     ? 'selected' : '' }}>🌿 Hadiah Kasual</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="ai-field">
                    <div class="ai-field-header">
                        ...
                        <label class="skip-toggle" for="skip-warna" title="Abaikan kriteria warna">
                            <input type="checkbox" id="skip-warna" name="skip_warna" value="1" class="skip-checkbox"
                                   data-target="color-picker-group"
                    ...
                    <div class="color-picker-grid" id="color-picker-group" style="{{ old('skip_warna') ? 'opacity:0.4; pointer-events:none;' : '' }}">
                        @php
                            $colorOptions = [
                                ['value' => 'bebas',   'label' => 'Bebas',    'hex' => 'linear-gradient(135deg,#e8a0a0,#d4b8d4,#c2f0c2,#f5deb3)'],
                                ...
                                ['value' => 'campur',  'label' => 'Mix',      'hex' => 'linear-gradient(135deg,#e8a0a0,#c2f0c2,#b0c4de)'],
                            ];
    ...
    <aside class="ai-info-panel" aria-label="Informasi sistem">
        ...
        <div class="info-card reveal delay-1">
            ...
            <div class="how-step">
                ...
                <div class="how-step-text"><strong>Isi Kriteria</strong> masukkan budget, acara, dan warna yang kamu suka. Bisa di-skip jika tidak relevan.</div>
            </div>
            <div class="how-step">
                <div class="how-step-num">2</div>
                <div class="how-step-text"><strong>AI Menghitung</strong> sistem Fuzzy Logic Mamdani mengevaluasi setiap buket dan memberikan skor kesesuaian.</div>
            </div>
            <div class="how-step">
                <div class="how-step-num">3</div>
                <div class="how-step-text"><strong>Hasil Terbaik</strong> buket diurutkan dari yang paling cocok. Klik untuk melihat detail dan pesan via WhatsApp.</div>
            </div>
        </div>

        {{-- ALGORITHM TAGS --}}
        <div class="algo-card reveal delay-2">
            <p class="algo-card-title">Engine AI</p>
            <div class="algo-tag-list">
                <span class="algo-tag accent">Fuzzy Logic</span>
                <span class="algo-tag accent">Mamdani</span>
                <span class="algo-tag">COG Defuzzifikasi</span>
                <span class="algo-tag">Multi-Kriteria</span>
                <span class="algo-tag">Skip Kriteria</span>
                <span class="algo-tag">Real-time Score</span>
            </div>
        </div>

        {{-- TIPS --}}
        <div class="algo-card reveal delay-3">
            <p class="algo-card-title">Tips Penggunaan</p>
            <div class="tip-item">Budget terlalu kecil? Aktifkan <strong>Skip Budget</strong> untuk lihat semua pilihan.</div>
            <div class="tip-item">Pilih warna <strong>Bebas</strong> jika tidak ada preferensi warna tertentu.</div>
            <div class="tip-item">Acara <strong>Romantis</strong> paling cocok untuk anniversary dan ungkapan perasaan.</div>
            <div class="tip-item">Skor di atas <strong>70</strong> berarti buket sangat cocok dengan kriteriamu.</div>
        </div>
...
@if(isset($products) && count($products) > 0)
    <section class="prev-results" aria-label="Hasil rekomendasi sebelumnya">
        <div style="margin-bottom: 40px;">
            ...
            <h2 style="font-family: var(--font-display); font-size: clamp(28px,3.5vw,44px); font-weight:300; color:var(--charcoal); margin-top:8px;">
                Rekomendasi <em style="font-style:italic; color:var(--rose);">Untukmu</em>
            </h2>
        </div>

        <div class="products-listing" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap:28px;">
...
<script>
    ...
    if (budgetSlider) {
        budgetSlider.addEventListener('input', function() {
            budgetVal.textContent = formatRupiah(this.value);
            inpBudget.value = this.value;
        });
    }
            ...
            if (target) {
                ...
                if (targetId === 'color-picker-group') {
                    ...
                    if (group) {
                        ...
                        group.style.pointerEvents = this.checked ? 'none' : '';
                        group.querySelectorAll('input').forEach(r => r.disabled = this.checked);
                    }
                }
            }
    ...
    document.querySelectorAll('.wa-order').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            window.open('https://wa.me/6282333000472?text=' + encodeURIComponent(msg), '_blank');
        });
    });
